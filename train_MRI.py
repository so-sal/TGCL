...
from utils.inference import visualize_max_cancer, visualize_small_patch, visualize_slices, inference_sliding_window
...
def main(args):
    ...
    estimated_steps_per_epoch = len(TRAIN_DATALOADER) // 2
    ...
    for epoch in range(1, args.num_epochs, 1):
        ...
        MODEL.eval()
        test_visualize(GENERATOR=TRAIN_GENERATOR, data_idx=3 , MODEL=MODEL, filename=f'visualization_log_MRI/RAUS_clsseg_patch{args.patch_size}_emb{args.embed_dim}_batch{args.batch_size}_epc{epoch}_train_[{train_loss:.3f}]_[{"-".join([f"{dice:.3f}"for dice in train_dice])}]_generator.png')
        test_visualize(GENERATOR=TRAIN_GENERATOR, data_idx=3 , MODEL=MODEL, filename=f'visualization_log_MRI/RAUS_clsseg_patch{args.patch_size}_emb{args.embed_dim}_batch{args.batch_size}_epc{epoch}_train_[{train_loss:.3f}]_[{"-".join([f"{dice:.3f}"for dice in train_dice])}]_batch.png', batch=[Img_subset, Label_subset, Crop_subset])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config_file', type=str, default='configs/config_ADC.yaml', help='Path to config file')
    ...
    parser.add_argument('--save_dir', type=str, default='model_weights_T2', help='directory of model weights')
    parser.add_argument('--weight_segmentation', type=float, default=0.99, help='segmentation loss weight')
    ...
    with open('configs/config_ADC.yaml', 'r') as f:
